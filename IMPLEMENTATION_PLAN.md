...
**Фаза 4 → модель эффективных настроек.** Расширить начатую для
`MappingMode` модель до assembly/root/map-level значений и общих правил
наследования `Default`, пока без наследования mapper-ов.
...
- [x] `MappingMode`.
  ...
  `Default` наследует root-level значение, а при его отсутствии разрешается в
  `MapNewAndExisting`; явное map-level значение имеет приоритет. Root-level
  настройка относится ко всему mapper-у независимо от порядка вызовов в
  линейном `Configure()`.
  ...
  неизвестным map-level значением пропускается, а неразрешимое root-level
  значение исключает конфигурацию mapper-а из генерации.
