...
Этот документ задаёт утверждённый порядок реализации и хранит текущее
состояние работ. Этапы выполняются сверху вниз. Более поздний этап не начинается,
пока не завершены предыдущие, если пользователь явно не изменил приоритет.

Каждый пункт реализуется отдельным полноценным TDD-срезом. Если в пункте
указано, что объём поддержки нужно согласовать отдельно, граница согласуется с
пользователем до написания тестов и production-кода. Актуализация,
инкрементальность и диагностики не переносятся в более ранние фазы.
...
**Фаза 2 → Простейший MapExisting.** Для destination без членов вернуть
переданный экземпляр без изменений.
...
- [ ] Простейший MapExisting.

  Destination без членов: вернуть переданный экземпляр без изменений.

- [ ] Convention mapping обычных свойств.
