...
- [x] Маркеры членов.
  ...
  с теми же правилами имени, доступности и совместимости. Он выполняется в
  automatic-фазе: сначала идут explicit expressions в порядке записи, затем
  `Auto()` и оставшиеся convention mappings в обычном порядке destination-
