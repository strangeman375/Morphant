...
Открытое решение пользователя — способ исправления S03-01–03. К этапу 4 автоматически не переходим. Публичная поправка про обе формы `new` уже в `main`; публикация внутренних материалов отдельно подтверждена пользователем после первоначального отказа автоматической проверки разрешений.
