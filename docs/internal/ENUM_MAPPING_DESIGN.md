...
Дата: 2026-09-23. Статус: согласованный дизайн обычных enum; реализация впереди.
Редакция 21: исследованы flags и возобновлено обсуждение их семантики;
новые предложения пока не утверждены. Пользователь утвердил композицию
правил обычных enum через IncludeBase:
локальные специальные ветки, базовые ветки от ближайшей базы, общая
конвенция и ближайшая завершающая ветка. Ложный guard продолжает поиск;
locals сохраняют область видимости и однократное вычисление. Все ранее
вынесенные спорные вопросы обычных enum закрыты. Flags обсуждаются отдельно.
...
Утверждены значения `Default`, `ByName`, `ByValue`, `ByValueAllowUndefined`.
...
После согласования обычных enum пользователь вернулся к flags.
[Исследование flags и варианты дизайна](ENUM_MAPPING_FLAGS_RESEARCH.md)
сопоставляет подходы мапперов, уточняет нерешённые вопросы и предлагает
альтернативы прежнему наброску ниже. Детали flags ещё не утверждены;
особенно открыты composite overrides и композиция числовой конвенции.
...
Прежний эскиз предлагал для whole-value overrides применять такое правило
...
Этот приоритет не утверждён. В [исследовании flags](ENUM_MAPPING_FLAGS_RESEARCH.md)
он сравнивается с вариантом единой побитовой семантики без whole-mask cases.
Нельзя реализовать его повторным
...
Для `E -> E` применяется выбранная стратегия так же, как для разных enum:
...
Новые настройки и `Inherited()` не добавляются. Не превращать inherited
`Convert`/factory в набор enum rules. Using callbacks могут наследоваться
...
| Два числовых режима | `ByValue` требует объявленного destination-значения с тем же числом, без требования к source; `ByValueAllowUndefined` сохраняет и неназванные числа в диапазоне destination | Имена и граница строгости утверждены пользователем; разделы 4 и 7 |
...
Актуальное исследование и варианты для согласования собраны в
[отдельном документе](ENUM_MAPPING_FLAGS_RESEARCH.md); новые предложения
