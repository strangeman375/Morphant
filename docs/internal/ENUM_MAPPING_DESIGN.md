...
Редакция 11: по уточнению пользователя ByName сравнивает имена без учёта
регистра через OrdinalIgnoreCase. Категория закрытых решений и оставшиеся
спорные вопросы сохраняются в разделе 10.
...
По последнему поручению пользователя зафиксированы решения первой категории:
сравнение имён без учёта регистра, числовое строковое представление, сохранение числа,
...
пользователь выбрал `OrdinalIgnoreCase`: имена сравниваются без учёта
регистра и независимо от culture. Например, `Active`, `active` и `ACTIVE`
совпадают. Это правило enum-конвенции; существующее сравнение имён object/
tuple members не меняется. Отдельная настройка ignore-case не вводится.
Неоднозначные совпадения разобраны в разделе 7. Выбор критерия по умолчанию
и способ сравнения имён — разные решения.
...
отключает конвенцию в `Members`. При предлагаемом приоритете завершающей
ветки `_ => result` сохраняет значение фабрики только для остатка после
...
Спорный приоритет этих правил выбирается отдельно; coverage анализирует
полученное поведение, а не устанавливает собственный порядок. Режим выбора
...
То же правило неоднозначности применяется к совпадениям без учёта регистра.
Если destination содержит `Ready = 10` и `READY = 20`, source-имя `Ready`
совпадает с обоими, поэтому нужен явный результат в `Members`. Не выбирать
по порядку объявления или точному регистру: конвенция использует
`OrdinalIgnoreCase`. Если все подходящие имена destination обозначают одно
число, enum-результат однозначен; aliases этого числа не создают конфликт.
...
регистр. `OrdinalIgnoreCase` управляет сравнением, а не форматированием:
...
направления независимы. Конвенция CLR-имён в string-to-enum также использует
`OrdinalIgnoreCase`: `"active"` соответствует `Active`. Применимость самой
`EnumMappingStrategy` этим не расширяется. Явные строковые patterns и
guards в `Members` сохраняют обычную C# семантику: ignore-case применяется
...
По поручению пользователя закрываются решения, продолжающие существующие
контракты либо уточняющие представление без дополнительного режима/API.
Канонические подробности остаются в указанных разделах.

| Вопрос | Зафиксированное решение | Основание и раздел |
|---|---|---|
| Сравнение имён `ByName` | `OrdinalIgnoreCase`; регистр не учитывается, culture не участвует | Уточнено пользователем; разделы 4 и 7. Выбор стратегии по умолчанию остаётся отдельным вопросом |
...
1. **Приоритет завершающей ветки.** Для одноимённого `Active` при `_ => Unknown`
   возможны оба результата: `Active` по конвенции либо `Unknown` по буквальному
   порядку C# switch. Предложение раздела 5 — конвенция перед fallback;
   оно ещё не утверждено.
2. **Граница mapping switch.** Нужно определить дополнение switch над `result`
   или `(source, result)` и смысл `Auto()` в нём. Нельзя незаметно подменять
   зарегистрированную пару на destination-to-destination либо заранее запускать
   преобразование ради governing value. Это влияет на наблюдаемое поведение,
   а не только на способ генерации.
3. **Flags.** Zero и composite overrides, guards/locals, fallback отдельного
...
4. **Границы числовой конвенции и identity.** Любое число или только допустимое
...
5. **Композиция switches через `IncludeBase`.** Сохраняет ли локальный fallback
...
6. **Library default стратегии.** `ByName` остаётся рекомендацией; `ByValue`
   — содержательная альтернатива. Это выбор поведения bare registration,
   поэтому его нельзя закрыть как техническую деталь сравнения имён.
...
| Names | OrdinalIgnoreCase независимо от culture; mixed-case enum/enum и string/enum; совпадения с разными и одинаковыми destination-числами; aliases; сохранение регистра enum-to-string; обычная C# семантика явных строковых patterns |
