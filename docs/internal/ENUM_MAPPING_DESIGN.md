...
Редакция 17: пользователь выбрал два числовых режима — строгий ByValue
и отдельное значение EnumMappingStrategy для сохранения неназванных чисел.
Имя ByValueAllowUndefined предлагается для второго режима. Пользователь
должен управлять реакцией на переполнение; предложена композиция через
Members без новой настройки. Границы строгости и same-type остаются ниже
предложениями. Flags по-прежнему отложены как отдельная тема.
...
Значения: `Default`, `ByName`, `ByValue` и отдельный ослабленный числовой
режим с предлагаемым именем `ByValueAllowUndefined`. Пользователь выбрал
строгий смысл `ByValue`; точное имя нового значения ещё не утверждено.
`Default` продолжает обычный поиск настройки. Утверждённый library default
для разных enum и enum-to-string — `ByName`. Числовой режим выбирается для
пары, mapper или через MSBuild по общим правилам precedence.
...
enum обсуждается вместе с identity ниже. Новый ослабленный режим требует
отдельно согласовать применимость к integer-to-enum: прежнее исключение
enum/integer из стратегии не отменяется молча. Для enum-to-string предложено
сохранить числовое форматирование обоих режимов без проверки source-имени;
это предложение рассмотрено в разделе 7.
...
числовых значения одной `EnumMappingStrategy`, без второго setting.
...
Выбор реакции на переполнение нужен; способ выразить его через имеющийся
DSL предлагается в разделе 7 и ещё требует согласования.
...
Обсуждение flags отложено пользователем до решения вопросов обычных enum.
Материал о flags сохраняет прежние направления и предложения для отдельного
обсуждения; они не определяют решения текущего этапа.
...
Для нового `ByValueAllowUndefined` предложен тот же числовой output:
у строки нет набора объявленных enum-значений, а принятый `ByValue` уже
сохраняет неназванные числа. Это предложение о применимости нового режима;
оно не добавляет проверку объявленности source и не меняет принятый output.
...
Решения по flags отложены отдельно.
...
`ByValue` или отдельный ослабленный режим;
...
- `ByValue` — строгая конвенция, учитывающая объявленные значения.
- Отдельное значение `EnumMappingStrategy` сохраняет число, даже если для
  него нет объявленной константы. Предлагаемое имя — `ByValueAllowUndefined`.

Оба режима соблюдают диапазон destination и сохраняют математическое число. Ослабленный режим
не означает unchecked cast и не разрешает усечение или смену знака.
Строгость относится к конвенции; явно написанные результаты `Members`
не получают скрытой проверки объявленности.

Предлагаемое уточнение строгости: проверять объявленность в destination,
без отдельного требования к source. Тогда неназванное source-число, уже
объявленное в destination, имеет соответствие. Эта граница ещё требует
согласования; обязательная объявленность с обеих сторон была бы другим
контрактом. Enum-to-integer сохраняет и неназванные значения в пределах
диапазона: у integer нет перечня объявленных вариантов.

Пользователь должен выбирать исключение либо fallback при переполнении.
Предлагаемый способ — считать непредставимое число отсутствием соответствия
на неявном пути и использовать существующие правила `Members`:
...
DSL marker. Предложение позволяет выбрать поведение конкретной пары;
отдельный setting `EnumOverflowHandling` не вводится.
Способ выбора реакции ещё не утверждён; требование дать такой выбор принято.
...
После выбора строгого и ослабленного режимов для `E -> E` предлагается
применять выбранную стратегию так же, как для разных enum: `ByName` и
строгий `ByValue` не сохраняют неназванное число автоматически; ослабленный
режим сохраняет. Прежняя рекомендация identity для неизвестных значений
при любой стратегии снята: она обходила бы явно выбранную строгость.
Общая same-type семантика ещё не утверждена. Explicit overrides выполняются
...
Например, два режима допустимости выбраны, но точные границы строгого
сопоставления и способ управления переполнением ещё открыты.
Наличие рекомендуемого варианта само по себе не делает его
...
| Два числовых режима | Строгий режим под `ByValue`; сохранение неназванных чисел — отдельное значение `EnumMappingStrategy` | Выбрано пользователем; разделы 4 и 7. Имя `ByValueAllowUndefined` и точная граница объявленности предложены, но не утверждены |
...
| Выбор реакции на переполнение | Пользователь должен иметь возможность выбрать исключение либо fallback | Требование пользователя; конкретный способ через `Members` предложен в разделе 7 и ещё не утверждён |
...
1. **Детали двух числовых режимов обычных enum.** Имя ослабленного режима;
   объявленность только destination или обеих сторон; общий контракт `E -> E`;
   применимость ослабленного режима к enum-to-string и integer-to-enum.
   Само наличие двух режимов и строгий смысл `ByValue` уже выбраны.
   Выбор реакции на переполнение нужен; предложено выразить его через
   существующий `Members`, включая отдельный guard для overflow. Этот способ
   пока не утверждён. Исследование связано с разделом 7.
...
По решению пользователя flags обсуждаются отдельно после обычных enum:
zero/composite overrides, guards/locals, fallback бита или маски, nullable
результат бита, полная маска `result` и атрибут только с одной стороны.
Сюда же относится допустимость ByValue при объявленном `Pair = 3` без
отдельных `1`/`2`. Предложения и примеры сохранены в разделе 7. Прежнее
направление на автоматическое определение `[Flags]` и единый `Members`
не отменено; детализация этой поддержки не блокирует обычные enum.
...
Полный охват наброска, включая отложенные flags: enum-to-enum, aliases, flags, nullable,
