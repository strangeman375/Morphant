...
Редакция 13: пользователь выбрал конвенцию перед завершающей веткой
в enum Members. MemberSelection.Explicit отключает неявную конвенцию.
Этот вопрос перенесён в закрытые решения; спорные вопросы — в разделе 10.
...
Зафиксированы решения: сопоставление имён в два этапа, конвенция перед
завершающей веткой, числовое строковое представление, сохранение числа,
...
Для mapping switch над source зафиксирован порядок:
...
1. **Граница mapping switch.** Нужно определить дополнение switch над `result`
...
2. **Flags.** Zero и composite overrides, guards/locals, fallback отдельного
...
3. **Границы числовой конвенции и identity.** Любое число или только допустимое
...
4. **Композиция switches через `IncludeBase`.** Сохраняет ли локальный fallback
...
5. **Library default стратегии.** `ByName` остаётся рекомендацией; `ByValue`
