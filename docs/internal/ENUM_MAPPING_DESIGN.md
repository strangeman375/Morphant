...
Дата: 2026-09-23. Статус: согласованный дизайн обычных enum и режимов flags;
реализация впереди. Редакция 22: пользователь утвердил FlagsMappingMode
со значениями Default, ByBit и ByMask, library default ByBit, независимость
от EnumMappingStrategy и единый режим итоговой пары после IncludeBase.
Members, Auto и fallback работают с выбранной единицей; Using всегда
получает полную маску. ByName для ByMask, строгая допустимость составных
масок, zero/null и границы применимости flags ещё требуют согласования.
Все ранее вынесенные спорные вопросы обычных enum закрыты.
...
применимость settings с Using и
границы обработки compiler warnings. Их канонические описания находятся
ниже; раздел 10 отделяет закрытые решения от открытых flags-вопросов.
...
разделе 7. Обе настройки независимы и используют обычные уровни precedence.
...
Для `ByMask` анализируются правила целых значений; семантика конвенции
ByName полной маски ещё открыта. Не перечислять все комбинации маски ради
анализа покрытия. Составные объявления и zero требуют уточнений раздела 7.

## 7. Flags, aliases, строки и числа

Пользователь согласовал два режима обработки flags. Исследование других
инструментов и оставшиеся альтернативы находятся в
[исследовании flags](ENUM_MAPPING_FLAGS_RESEARCH.md). Этот раздел —
канонический контракт выбранных режимов; отдельно помечены ещё открытые детали.
...
Основной согласованный сценарий — flags enum → flags enum; границы других
пар перечислены ниже среди открытых вопросов.
...
число целиком. Для ByName ещё нужно выбрать: сопоставлять только целые
именованные значения или также строить маску по именам составляющих битов.
Ни один вариант пока не утверждён. Возможное внутреннее разбиение конвенции
не должно означать побитового выполнения Members в режиме ByMask.
...
Для ByBit сохраняется предложение об обходе битов от младшего к старшему
в точной ширине source underlying type, включая signed high bit. Этот
порядок ещё нужно закрепить вместе с zero и nullable результатом бита.
...
Допустимость целой неназванной комбинации при строгом ByValue остаётся
открытой; согласованный Pair = 3 не выбирает автоматически общий критерий.

### Открытые детали flags

- ByName для полной маски: exact named value либо сборка по именам битов,
  без повторного запуска Members. Нужно определить поведение composites
  и aliases в выбранном варианте.
- Строгий ByValue для неназванных целых масок в ByMask и integer-to-flags:
  комбинации одиночных объявленных битов плюс exact declared composites
  либо OR целых объявленных значений. Сравнение с более широким критерием
  всех известных битов приведено в исследовании flags. Выбор пока не сделан.
- Zero: прежнее предложение — нулевой результат по конвенции и один проход
  правил с source = 0, чтобы работало None → другое значение. Ноль не
  добавляется к ненулевым маскам. Взаимодействие с Explicit ещё не утверждено.
- Nullable результат бита: null не равен нулевой маске. Предложение о terminal
  null без выполнения следующих битов ещё нужно согласовать.
- Composite case непосредственно над битом в ByBit недостижим; предложение
  диагностировать его ещё требует выбора границы анализа. Pattern над полным
  result или вычисленным выражением может быть достижимым и не запрещается
  только из-за составной константы.
- Применимость при flags только с одной стороны и к другим видам пар,
  включая диагностику неприменимой FlagsMappingMode, ещё требует уточнения.
  Не распространять OR автоматически на ordinary enum, string или object.
- Enum/integer и числовой flags-to-string сохраняют направление на обработку
  целого числа; это не выбор нового побитового integer/string алгоритма.
  Формат списка flags-имён и обратный parsing пока вне согласованного объёма.
...
Решения по flags обсуждаются отдельно в
...
ещё открыт; варианты перечислены в разделе FlagsMappingMode.
...
| Числовая строка | `ByValue` и `ByValueAllowUndefined` дают invariant decimal с точной знаковостью/шириной, включая неназванные числа; прежнее направление flags — формат всей маски | Оба режима утверждены пользователем; у строки нет набора enum-констант; раздел 7 |
...
Все ранее вынесенные спорные вопросы обычных enum согласованы, включая
композицию switches через `IncludeBase`. Проверки реализации выбранного
контракта остаются необходимыми; flags-семантика обсуждается отдельно.

### Текущее обсуждение: flags

Выбор единицы правил, fallback и числовой проверки закрыт двумя режимами
FlagsMappingMode. Точные composite cases относятся к ByMask; смешанный проход
Members над маской и битами снят. Случай Pair = 3 имеет разные согласованные
ответы при ByBit и ByMask.

Открыты ByName для полной маски, строгая допустимость неназванных составных
масок, zero и nullable результат бита, порядок битов и детали диагностики,
а также границы применимости при flags только с одной стороны и других
видах пар. Подробности — в разделе 7 и
[исследовании flags](ENUM_MAPPING_FLAGS_RESEARCH.md). Эти предложения
не следует читать как утверждённый контракт.
...
Полный охват наброска, включая обсуждаемые flags: enum-to-enum, aliases, flags, nullable,
...
| Flags | Attribute detection; ByBit default и явный ByMask; независимость от стратегии и обычное наследование режима; один режим всех уровней IncludeBase; единица source/Auto/fallback/вычислений; точные composite cases только при подходящем входе; Pair = 3 и signed -1 при разных ширинах; zero/null; неизвестные биты; aliases; отсутствие смешанного прохода и скрытой проверки explicit результатов |
