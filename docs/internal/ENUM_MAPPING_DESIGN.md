...
Редакция 20: пользователь утвердил композицию правил через IncludeBase:
...
вынесенные спорные вопросы обычных enum закрыты. Flags отложены отдельно.
...
ниже; раздел 10 отделяет закрытые решения от отложенных flags-вопросов.
...
контракта остаются необходимыми; flags-семантика отложена отдельно.
