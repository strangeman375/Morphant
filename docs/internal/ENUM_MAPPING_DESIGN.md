...
Редакция 16: по решению пользователя flags отложены как отдельная тема;
сначала согласовываются обычные enum. Неизвестные числа, E → E и диапазон
исследованы отдельно; новые рекомендации ещё не утверждены. Принятая
привязка конвенции и Auto() к source текущей пары сохраняется.
...
Сравнение популярных мапперов и актуальные рекомендации для обычных enum:
[неизвестные числа, identity и диапазон](ENUM_MAPPING_NUMERIC_RESEARCH.md).
Эти рекомендации требуют согласования; решения по flags отложены отдельно.
...
В одном из обсуждаемых вариантов конвенция с ordinary enum destination
...
1. **Границы числовой конвенции и identity обычных enum.** Любое представимое
   число или только объявленное destination-значение; исключение либо fallback
   при выходе из диапазона; неназванные значения при `E -> E`. Отсутствие
   потери числа закрыто, но не выбирает политику допустимости. Новая validation
   setting не предлагается. Исследование и рекомендации связаны с разделом 7.
