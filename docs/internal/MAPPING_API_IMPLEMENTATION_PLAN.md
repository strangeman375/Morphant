...
данные, но не mapping operation, lifecycle либо область применимости
конфигурации. Superseded overload-ы и прежнее arity-dependent поведение не
сохраняются compatibility-слоем: до реализации весь generated surface,
semantic model, lowering, XML documentation, exact-source snapshots и
consumer scenarios обновляются одним coherent срезом.
...
`Option.None` / `Option.Some`, но не отличает `Create` от `Update(null)`.
Полная форма дополнительно получает исходную operation и scoped mapper.
...
Срез реализован и принят пользователем 11 августа 2026 года. Он не
...
slice реализован и принят пользователем 12 августа 2026 года.
...
**Core v0: реализация завершена.**

Статус: функциональные этапы, observable failures и финальный двусторонний
аудит diagnostics приняты пользователем. Следующий implementation stage не
выбран; предрелизная полировка выполняется отдельными согласованными срезами.
...
Статус: полный каталог, предварительное выравнивание production model,
vertical slices категорий 1–12 и финальный двусторонний аудит реализованы и
