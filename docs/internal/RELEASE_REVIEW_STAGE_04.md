...
Статус: аудит и согласованные исправления завершены. S04-01–S04-05 закрыты
в проверенной области 2026-09-06; общее правило именования и алиасы членов
согласованы с пользователем и внедрены. [Итог исправлений и новые проверки](RELEASE_REVIEW_STAGE_04_FIXES.md).
Ниже сохранены находки исходного аудита и утверждённый контракт. Этап 5 не начат.

## Версия, восстановление и доказательства

Исходный аудит проверял версию [2695e07](https://github.com/strangeman375/Morphant/commit/2695e07429f468243dcfaeb90d5c99ca8ca874ba),
tree `9551b0e19f015a8ce589a4e84b9e805944eddf94`. Production-код и постоянные
тесты при первоначальной проверке не менялись; последующие исправления
и их точная версия описаны в отдельном итоговом отчёте.
...
При восстановлении аудита полную Release-сборку, все unit/integration
и multi-OS CI повторно не запускали. Новые локальные прогоны после исправлений
приведены в [итоговом отчёте](RELEASE_REVIEW_STAGE_04_FIXES.md).
...
Исправлено: обычное назначение и проверка полноты сохраняют исходные имена.
Для явного Members пользователь согласовал алиасы с подчёркиваниями; record
и `with` сохранены. Проверены Create/Update, strict validation, переименование
destination и занятые имена алиасов; подробности в итоговом отчёте.
...
на проверенной версии была приведена эта форма. C# связывает target-typed new с `Option<Destination>`
...
Исправлено: все три страницы используют block body. После S04-04 варианты
с TryGetValue тоже проходят. Три текущих примера извлечены из Markdown,
скомпилированы как обычный C# 9 consumer и прошли 12 runtime-проверок.
...
Исправлено: mapping-тип проверяется отдельно от его containing scopes;
сам static/ref-like root остаётся недопустимым generic-аргументом. Проверены
source/destination, независимая пара, несколько уровней и generic-контейнеры,
включая обычные вложенные типы внутри ref struct.
...
Исправлено: учитывается связанный TryGetValue именно параметра previous.
Сохраняются short-circuit, out-присваивание, область видимости, nullable flow
и число вычислений. Регрессии Create, Update(null), reuse, replacement
и формы ветвления прошли; compiler-проверки выполнены на Roslyn 4.4.0 и 4.9.2.
...
### Согласованное общее правило именования

Утверждено пользователем и внедрено 2026-09-06.
Таблица выше описывает результат аудита прежней реализации.

Все сгенерированные construction/member/constructor-parameter типы используют
одну схему, независимо от вида destination:

```text
Morphant.Generated.N_<id>.OrderConstruction
Morphant.Generated.N_<id>.OrderMembers
Morphant.Generated.N_<id>.OrderConstructorParameters
```

`<id>` — стабильный 128-битный хеш, записанный 32 строчными шестнадцатеричными
цифрами. Используется существующий способ получения 128-битного значения:
первые 16 байт SHA-256 от UTF-8 ключа. Namespace имеет три сегмента и длину
53 символа. Отдельные `A_<assembly>`, исходные namespace/type scopes,
`Types`, `Tuples` и `Plans` больше не нужны.

Ключ включает простое имя и public key token генерирующей сборки, а также
полную идентичность destination. Изоляция сборок сохраняется внутри хеша.
Для обычных типов различаются исходный namespace, цепочка содержащих типов,
metadata-имена и generic arity. Generic destination определяется по definition,
чтобы сохранялся один generated generic-тип для разных подстановок.
Одинаковые destination-планы переиспользуются разными мапперами одной сборки.

Для кортежей ключ сохраняет физический контракт и полное рекурсивное
представление: имена элементов, nullable-аннотации, `dynamic`, вложенность
и аргументы типов. Разные представления не объединяются по одной CLR-паре.
Имена tuple-типов остаются `TupleConstruction`, `TupleMembers` и
`TupleConstructorParameters`; имена обычных типов сохраняют имя destination.

Namespace должен оставаться неизменным при повторной сборке, смене версии
сборки или зависимости и перестановке регистраций. В ключ не входят version,
MVID, TFM, facade assemblies, пути, mapper identity, случайные значения
и порядок обхода. Читаемость обеспечивают короткое имя типа и описание
destination в XML-документации; namespace служит технической идентичностью.

Обе формы `new(...)` и явное имя generated-типа поддерживаются.
При переходе на новое правило явные `using`, aliases и полные имена нужно обновить.
Один namespace на destination может потребовать больше `using`, чем прежняя
группировка по исходному namespace. DSL-расширения остаются в `Morphant`,
а partial-объявления мапперов — в пользовательском namespace.
Правило имён hint-файлов не меняется.

Критерии завершения: одинаковая схема предварительной и итоговой генерации;
полные snapshots; явные имена через using/alias/FQN и target-typed new;
ordinary/nested/generic/tuple destinations; различия tuple presentation;
IVT с source/DLL/reference assembly; подпись, пунктуация и Unicode в имени
сборки; стабильность при смене версий и порядка регистраций; отсутствие
лишнего дублирования типов. Эти проверки выполнены; результаты и измерения
нового вывода приведены в [итоге исправлений](RELEASE_REVIEW_STAGE_04_FIXES.md).
По решению пользователя от 2026-09-07 tuple XML summary сохраняет краткую
ссылку на определение типа с generic arity, без перечисления элементов.
...
Разрешённые исправления S04-01–S04-04, namespace-дизайн S04-05 и уточнение
XML-описаний внедрены. Алиасы конфликтующих членов отдельно согласованы
с пользователем; форма record сохранена. Постоянные compiler/runtime-регрессии
следуют [TESTING_GUIDELINES.md](TESTING_GUIDELINES.md).
Release и оба test projects прошли на исправленной версии. Переход к этапу 5
ожидает отдельной команды пользователя.
