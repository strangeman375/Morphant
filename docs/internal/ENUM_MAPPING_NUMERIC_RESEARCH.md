...
Дата: 2026-09-22. Статус: исследование; выводы обновлены после выбора
пользователем двух числовых режимов, со строгим смыслом `ByValue`.
...
Пользователь выбрал поддержку обоих сценариев: только объявленные значения
и сохранение неназванного числа. Строгий вариант получает имя `ByValue`,
ослабленный — отдельное значение `EnumMappingStrategy`. Предлагаемое имя
`ByValueAllowUndefined` пока не утверждено. Общий default остаётся `ByName`.
Оба числовых режима сохраняют уже принятое требование не терять число.
...
стратегии снята. Предложено подчинять эту пару выбранной стратегии, чтобы
копирование не обходило строгий режим. Same-type контракт ещё не утверждён.

Пользователь также потребовал выбор реакции на переполнение. Предложено
выражать его через `Members`: завершающий fallback, явный `Auto()`/throw
либо отдельный guard для overflow перед fallback неизвестных значений.
Это не общий контракт исследованных библиотек и не утверждённая форма API.

Канонические подробности, примеры и оставшиеся границы решения:
[числовая конвенция](ENUM_MAPPING_DESIGN.md#числовая-конвенция).
Там отдельно отмечены объявленность source/destination, имя нового режима,
same-type и применимость к другим направлениям. Наличие двух режимов уже
выбрано; эти уточнения не переоткрывают сам выбор.
