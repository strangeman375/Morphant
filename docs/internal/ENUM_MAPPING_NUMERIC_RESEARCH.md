...
их имена, destination-only строгость, same-type и выбор реакции на переполнение.
...
Общий default остаётся `ByName`. Оба числовых режима сохраняют требование
не терять число. В enum-to-string оба форматируют число, включая неназванное.
...
Применимость числовых режимов к integer-to-enum ещё требует согласования:
прежняя граница настройки не расширяется автоматически вслед за этим выбором.
