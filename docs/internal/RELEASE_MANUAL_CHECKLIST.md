...
- Workflow `CI` на каждом pull request и push в `main` выполняет Release build,
  ...
  4.9.2, package consumer на минимальном SDK 7.0.100 / MSBuild 17.4 и coverage
  gates для runtime, generator и build tasks. На push в `main` он дополнительно
  прогоняет полный
...
- Ветка `main` защищена от прямых изменений не-владельцами: для них требуется
  ...
  `Roslyn 4.9.2 compatibility`,
  `Package consumer (.NET SDK 7.0.100 / MSBuild 17.4)`, а также разрешение
  всех review discussions.
...
- [ ] **PKG-09 — buildTransitive содержит settings и Git snapshot lifecycle**
  ...
  - Ожидаемый результат: ровно шесть properties:
    ...
    по умолчанию сохраняет только `TypeMapper`, а при
    `MorphantGitSnapshotDetail=Full` — весь generated surface,
...
- [ ] **PKG-11 — Все product assemblies strong-named**
  - Исходное состояние: runtime, generator и build-task DLL извлечены из
    PACKAGE.
  - Действие: просмотреть assembly identity через Rider/decompiler либо
    `AssemblyName.GetAssemblyName(...).GetPublicKeyToken()`.
  - Ожидаемый результат: у всех трёх assemblies непустой public key и
    одинаковый public key token `ba27fb6be8f80649`.

- [ ] **PKG-12 — Assembly versions стабильны**
  - Исходное состояние: все три DLL из PACKAGE.
  - Действие: проверить assembly, file и informational versions.
  - Ожидаемый результат: assembly version — `0.1.0.0`; product/package
    informational version соответствует 0.1.0 и RC commit; runtime, generator
    и build task не расходятся по product identity.
...
- [ ] **PKG-13 — Symbol package имеет точный payload**
  ...
  - Ожидаемый результат: остаются только `Morphant.nuspec` и непустой
    `lib/netstandard2.0/Morphant.pdb`; отдельных generator/build-task PDB нет,
    потому что обе assemblies используют embedded debug information.
...
- [ ] **USE-02 — Quick-start mapper виден в Rider**
  - Исходное состояние: C9 содержит types и mapper, дословно скопированные из
    root README; включён `MorphantGitSnapshot`, mapping использует `Members`, а
    mapper snapshot уже создан до открытия solution.
  - Действие: открыть solution, дождаться design-time analysis, проверить
    completion для `Members`, выполнить Restart Roslyn Analyzers and Source
    Generators, открыть generated configuration API и partial mapper.
  - Ожидаемый результат: `[MorphantMapper]` распознан; generated partial class
    ...
    generated extension methods остаются доступны; snapshot не диагностируется
    как compiler input и не вызывает `MORPH0034`; generated documents не
...
- [ ] **INC-01 — Cold generation полна и детерминирована**
  ...
  - Ожидаемый результат: набор имён и содержимое полностью совпадают, не
    ...
    `// <auto-generated />`, включают `#nullable enable`, сохраняют выбранное
    Roslyn UTF-8 representation и используют CRLF.
...
- [ ] **INC-09 — Git snapshot синхронизируется после успешной компиляции**
  ...
  - Действие: добавить и удалить mapping; выполнить no-op build и компиляцию с
    ...
    удалить один TFM; проверить default `Mappers`, переключить detail в `Full`
    и обратно; повторить multi-target build с parallel MSBuild nodes; проверить
    checkout при `.gitattributes` rule для Morphant CRLF.
  - Ожидаемый результат: stale files удаляются только после успешного compiler
    ...
    по правилу last-successful-build-wins; default snapshot содержит только
    mapper implementations, `Full` добавляет API/template artifacts, возврат к
    `Mappers` удаляет их; unrelated files сохраняются; checkout не создаёт
    EOL-only rewrite или timestamp churn.
...
- [ ] **COMP-09 — Минимальный compiler/MSBuild host потребляет package**
  - Исходное состояние: PACKAGE собран текущим release SDK; отдельный consumer
  ...
  - Ожидаемый результат: runtime, analyzer и `Morphant.Build.Tasks.dll`
    ...
    содержит только mapper implementation и не создаёт дополнительных
    manifest/state-файлов.
