...
- Generated files use deterministic CRLF line endings outside literal text,
  start with `// <auto-generated />`, and contain `#nullable enable`.
  Preserve original line endings inside multiline interpolated strings so
  formatting cannot change their values.
- Hint names use
  `Morphant.Generated.<ArtifactKind>.<ReadableLabel>__<identity>.g.cs`, with a
  singular artifact kind and a permanent 128-bit ID (32 lowercase hexadecimal
  digits). Compute the ID from the complete stable entity identity, before
  sanitizing or shortening the readable label. Limit the whole filename to
  220 UTF-8 bytes by truncating only the label; preserve the ID and suffix.
  Names must not depend on other registrations, allocation order, assembly or
  dependency versions, paths, target frameworks or random values.
- Construction and member files reuse their destination namespace ID. Mapper
  ...
  filenames for uniqueness. Approved on 2026-09-15.
...
- Generated-name tests must cover permanent IDs, stability across adjacent
  ...
  Generated-surface snapshots must use complete literal expected sources and
