namespace Morphant.Generator.IntegrationTests.TypeMapperNestedMapTests;

[TestFixture]
internal sealed class AdaptiveTests
{
    [Test]
    public void Uses_available_destinations_in_all_four_forms_and_checks_generic_destination()
    {
        global::Morphant.Generator.IntegrationTests.CSharp9.Scenarios.Adaptive_4f7bdae8.Scenario.Verify();
    }

    [Test]
    public void Uses_the_actual_replacement_member_for_adaptive_update()
    {
        global::Morphant.Generator.IntegrationTests.CSharp9.Scenarios.Adaptive_9e31b559.Scenario.Verify();
    }

    [Test]
    public void Casts_a_broad_current_member_to_a_value_destination()
    {
        global::Morphant.Generator.IntegrationTests.CSharp9.Scenarios.Adaptive_94672b4d.Scenario.Verify();
    }
}
