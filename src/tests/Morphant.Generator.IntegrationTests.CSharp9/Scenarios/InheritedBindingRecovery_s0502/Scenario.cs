...
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.InheritedBindingRecovery_s0502
{
    ...
    {
        protected virtual string Secret(string value) => "secret:" + value;
    ...
    {
        protected new string Secret(string value) =>
            throw new InvalidOperationException("The hidden member must never execute.");
    ...
    public static class Scenario
    {
            ...
            TDestination destination)
        {
            ...
            ExpectConfigurationFailure<TSource, TDestination>(
                () => typed.Create(source, default), MappingOperation.Create);
            ExpectConfigurationFailure<TSource, TDestination>(
                () => typed.Update(source, default!, default), MappingOperation.Update);
            ExpectConfigurationFailure<TSource, TDestination>(
                () => typed.Update(source, destination, default), MappingOperation.Update);
        }

        private static void ExpectConfigurationFailure<TSource, TDestination>(
            Action action, MappingOperation operation)
        {
            try
            {
                action();
            }
            catch (MappingConfigurationException error)
            {
                if (error.Operation != operation || error.SourceType != typeof(TSource) ||
                    error.DestinationType != typeof(TDestination) || string.IsNullOrEmpty(error.Reason))
                    throw new InvalidOperationException("Incorrect typed recovery metadata.", error);
                return;
            }

            throw new InvalidOperationException(
                "An inherited callback with unavailable binding was executed.");
        }
    }
}
