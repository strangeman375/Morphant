...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.ResolveInterpolation
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Resolve((input, previous) => { if(previous.HasValue) return previous.Value; return new(string.Join(",", from value in input.Values select $"{value}")); });
    }
