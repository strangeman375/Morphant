...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.MembersInterpolation
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Construct(input => new("")).Members(input => new() { Value = string.Join(",", from value in input.Values select $"{value}") });
    }
    ...
    public static class Scenario
    {
        public static void Verify()
        {
            ...
            if (destination.InitialValue != "" || destination.Value != "1,2,2")
            ...
            if (!ReferenceEquals(updated, destination) ||
                updated.InitialValue != "" || updated.Value != "3")
