...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.TupleLocal
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Construct(input => { var context = input.Values[0]; return new(string.Join(",", from value in input.Values select (context, Offset: value) into row select row.context + row.Offset)); });
    }
    ...
    public static class Scenario
    {
        public static void Verify()
        {
            ...
            if (destination.InitialValue != "2,3,3" || destination.Value != "2,3,3")
            ...
            if (!ReferenceEquals(updated, destination) ||
                updated.InitialValue != "2,3,3" || updated.Value != "2,3,3")
