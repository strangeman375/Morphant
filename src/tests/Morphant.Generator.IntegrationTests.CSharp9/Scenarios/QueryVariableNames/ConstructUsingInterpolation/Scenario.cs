...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.ConstructUsingInterpolation
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .ConstructUsing(input => new Destination(string.Join(",", from value in input.Values select $"{value}"))).Members(input => new() { Value = string.Join(",", from context in input.Values select $"{context}") });
    }
