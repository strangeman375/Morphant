...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.ResolveReplacementInterpolation
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Resolve((input, previous) => { if (previous.HasValue && input.Values.Length == 0) return previous.Value; return new(string.Join(",", from value in input.Values select $"{value}")); });
    }
    ...
    public static class Scenario
    {
        public static void Verify()
        {
            ...
            if (ReferenceEquals(updated, destination) ||
                updated.InitialValue != "3" || updated.Value != "3")
                throw new global::System.InvalidOperationException("Update changed binding or result identity.");

            source.Values = new int[0];
            var reused = mapper.Update(source, updated, default);
            if (!ReferenceEquals(reused, updated) || reused.Value != "3")
                throw new global::System.InvalidOperationException("Resolve lost its reuse branch.");
        }
    }
}
