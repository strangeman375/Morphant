...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.TupleParameter
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Construct(input => new(string.Join(",", from value in input.Values select (input, Offset: value) into row select row.input.Values.Length + row.Offset)));
    }
    ...
    public static class Scenario
    {
        public static void Verify()
        {
            ...
            if (destination.InitialValue != "4,5,5" || destination.Value != "4,5,5")
            ...
            if (!ReferenceEquals(updated, destination) ||
                updated.InitialValue != "4,5,5" || updated.Value != "4,5,5")
