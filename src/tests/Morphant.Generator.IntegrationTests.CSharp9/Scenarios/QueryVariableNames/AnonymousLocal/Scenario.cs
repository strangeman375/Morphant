...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.AnonymousLocal
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Construct(input => { var context = input.Values[0]; return new(string.Join(",", from value in input.Values select new { context } into row select row.context)); });
    }
    ...
    public static class Scenario
    {
        public static void Verify()
        {
            ...
            if (destination.InitialValue != "1,1,1" || destination.Value != "1,1,1")
            ...
            if (!ReferenceEquals(updated, destination) ||
                updated.InitialValue != "1,1,1" || updated.Value != "1,1,1")
