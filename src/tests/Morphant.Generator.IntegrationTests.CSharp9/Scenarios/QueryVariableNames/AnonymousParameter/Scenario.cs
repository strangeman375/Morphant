...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.AnonymousParameter
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Construct(input => new(string.Join(",", from value in input.Values select new { input } into row select row.input.Values.Length)));
    }
    ...
    public static class Scenario
    {
        public static void Verify()
        {
            ...
            if (destination.InitialValue != "3,3,3" || destination.Value != "3,3,3")
            ...
            if (!ReferenceEquals(updated, destination) ||
                updated.InitialValue != "3,3,3" || updated.Value != "3,3,3")
