...
using System.Linq;
using Morphant;
namespace Morphant.Generator.IntegrationTests.CSharp9.Scenarios.QueryVariableNames.NestedInterpolation
{
    ...
    public partial class Mapper : TypeMapper<Mapper>
    {
        protected override void Configure(MapperBuilder builder) =>
            builder.Map<Source, Destination>()
                .Construct(input => { var offset = input.Values[0]; return new($"[{string.Join(",", from value in input.Values select $"{value + offset}") }]"); });
    }
