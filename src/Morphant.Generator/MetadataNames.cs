namespace Morphant.Generator;

internal static class MetadataNames
{
    public const string MorphantMapperAttribute =
        "Morphant.MorphantMapperAttribute";

    public const string TypeMapper =
        "Morphant.TypeMapper`1";

    public const string TypeMapperInterface =
        "Morphant.ITypeMapper`2";

    public const string MapperBuilder =
        "Morphant.TypeMapper`1+MapperBuilder";

    public const string MapperBuilderBase =
        "Morphant.MapperBuilderBase`1";

    public const string PairMapperBuilder =
        "Morphant.MappingBuilder`3";

    public const string MapperDeclaration =
        "Morphant.IMapperDeclaration";

    public const string Member =
        "Morphant.Members.Member`1";

    public const string ConstructorParameter =
        "Morphant.Members.ConstructorParameter`1";

    public const string GenericValueMarker =
        "Morphant.Markers.ValueMarker`1";

    public const string MemberMarker =
        "Morphant.Markers.MemberMarker";

    public const string ConstructorMarker =
        "Morphant.Markers.ConstructorMarker";

    public const string MappingContextMarker =
        "Morphant.Context.MappingContextMarker";

    public const string SystemType =
        "System.Type";
}
